...
export async function findMSBuild() {
    ...
    if (matches.length > 1) {
        Log.log("warn", "find-msbuild", `find ${formatNumber(matches.length)} MSBuild version: [${matches.map((path) => light(path)).join(", ")}]`);
    }
    const pbuilds = matches.map(async (selected) => {
        ...
        if (!m) {
            Log.log("warn", "find-msbuild", `${selected} is not a valid msbuild path, can not find it's versdion`);
            return {
                path: selected,
                version: 0,
            };
        ...
        Log.log("info", "find-msbuild", `discover msbuild v${formatNumber("%s")} in ${highlight("%s")}`, m[1], selected);
        return {
            path: selected,
            version: Number(m[1]),
        };
